...
本文记录 Quartz 框架中“最近笔记”（Recent Notes）组件排序异常的原因及解决方案，主要涉及 Git 提交记录作为时间元数据的优先级配置。
...
1. **文件系统日期重置**：GitHub Actions 在执行构建时会重新 Checkout 仓库代码，导致所有文件的系统修改时间（FileSystem Date）被更新为流水线运行的当前时间。
