...
* **报错崩溃**：`Import "Profile" will always be undefined`。
* **样式不对/头像加载失败**：本地有头像，推上去就变成了破损图片，或者干脆没有自定义布局。
...
2. **子模块远程地址没改**：`.gitmodules` 里的 URL 还是原作者的仓库，Actions 会跑到原作者那里找你的 Commit，结果必然报错。
...
2. **同步主目录布局**：确保根目录下的 `quartz.layout.ts` 也同步了修改。
3. **处理静态资源路径**：在组件里使用 `resolveRelative(fileData.slug!, opts.avatar)` 来解析相对路径，确保子路径部署也能加载图片。
4. **更新主仓库引用**：
    * `cd ..`
    * `git add quartz quartz.layout.ts`
    * `git commit -m "feat: sync everything"`
    * `git push`

## 4. 常见报错小结

* **fatal: not our ref...**：子模块代码没 push 上去，或者 `.gitmodules` 地址还是官方的。
* **头像显示不出来**：绝对路径在 GitHub Pages 子路径下失效了，改用相对路径解析。
* **改了布局没效果**：检查一下是不是只改了子模块里的 layout 而没同步到根目录。
